...
def plot_aperture(
    ...
    **kwargs,
    ...
    nicely the lattice layout and the aperture tolerance across the machine.
    Beware: this function assumes the user has previously made a call to the `APERTURE` command in `MAD-X`.
    ...
    madx.command.twiss()
    twiss_df = madx.table.twiss.dframe().copy()
    aperture_df = pd.DataFrame.from_dict(dict(madx.table.aperture))  # slicing causes issues with .dframe()
    twiss_df.s = twiss_df.s - xoffset
    ...
    aperture_axis.plot(aperture_df.s, aperture_df.n1, marker=".", ls="-", lw=0.5, label="Aperture Limits")
    aperture_axis.fill_between(aperture_df.s, aperture_df.n1, aperture_df.n1.max(), interpolate=True)
