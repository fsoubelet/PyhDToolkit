...
class ListOperations:
    ...
    def union_by(lst_1: list, lst_2: list, function) -> list:
        ...
        element of both.
        Create a set by applying fn to each element in a, then use list comprehension in combination with fn on b to only
        keep values not contained in the previously created set, _a.
...
class MiscellaneousOperations:
    ...
    def map_values(obj: dict, function) -> dict:
        ...
        Use dict.keys() to iterate over the object's keys, assigning the values produced by function to each key of a new
        object.
        """
        ret = {}
        for key in obj.keys():
