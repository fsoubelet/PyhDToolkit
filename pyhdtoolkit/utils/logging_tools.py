...
This is entirely copy-pasted from the pylhc/omc3 library, so that I can use this functionnality without importing it,
since it is a very consequence package. All credits go to the pylhc/omc working team for creating this.
"""
...
class MaxFilter:
    ...
    def filter(self, log_record):
        """Determine if the log level is <= to set max printable level."""
...
class DebugMode:
    ...
    def __exit__(self, value, traceback):
        if self.active:
            ...
            log_id = "" if self.log_file is None else "'{:s}'".format(os.path.basename(self.log_file))
...
class TempFile:
    ...
    It won't open the file, so only the file's path is returned.
...
def get_logger(name, level_root=DEBUG, level_console=INFO, fmt=BASIC_FORMAT):
    ...
    if name == "__main__":
        ...
        root_logger.addHandler(stream_handler(level=level_console, fmt=fmt, max_level=INFO))

        # print console warnings
        root_logger.addHandler(stream_handler(level=max(WARNING, level_console), fmt=fmt, max_level=WARNING))

        # print errors to error-stream
        root_logger.addHandler(stream_handler(stream=sys.stderr, level=max(ERROR, level_console), fmt=fmt))
