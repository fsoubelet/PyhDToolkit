...
from pyhdtoolkit.utils.operations import ListOperations, MiscellaneousOperations, NumberOperations, StringOperations
...
class TestAttrDict:
    def test_attrdict_default_none(self):
        assert AttrDict() == {}

    def test_dotdict_set_attribute(self):
        attr_dict = AttrDict()
        attr_dict.a = 1
        assert attr_dict["a"] == 1

    def test_dotdict_access_attribute(self):
        attr_dict = AttrDict()
        attr_dict["a"] = 1
        assert attr_dict.a == 1

    def test_attrdict_update(self):
        attr_dict = AttrDict()
        attr_dict.a = 1
        attr_dict.update({"b": 2})
        assert attr_dict == {"a": 1, "b": 2}

    def test_attrdict_attribute_error(self):
        with pytest.raises(AttributeError):
            assert AttrDict().foo

    def test_attrdict_key_error(self):
        with pytest.raises(KeyError):
            assert AttrDict()["foo"]

    def test_attrdict_del_attribute(self):
...
class TestListOperations:
    ...
    @pytest.mark.parametrize(
        "array, size, error", [(list(range(10)), 0, ZeroDivisionError), (5, 20, TypeError), ([], "string", TypeError)],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([], []),
            ([[1, 2], [None], [True, False], [_square, "string"]], [1, 2, None, True, False, _square, "string"],),
        ],
    )
...
class TestMiscellaneousOperations:
    ...
    @pytest.mark.parametrize(
        "obj, func, result",
        [
            ({"a": 0, "b": 1, "c": 2, "d": 3}, lambda x: x % 2 == 0, {"a": True, "b": False, "c": True, "d": False},),
            (
                {"a": list(range(5)), "b": list(range(10)), "c": list(range(15))},
                lambda x: len(x),
                {"a": 5, "b": 10, "c": 15},
            ),
            ({"a": 0.5, "b": 0.9, "c": 1.2, "d": 2.1, "e": 25}, math.floor, {"a": 0, "b": 0, "c": 1, "d": 2, "e": 25},),
        ],
    )
...
class TestMultiThreaderExecutor:
    ...
    def test_multithreading(self, function, inputs, results, threads):
        MultiThreader.execute_function(func=function, func_args=inputs, n_threads=threads) == results
...
class TestNumberOperations:
    ...
    @pytest.mark.parametrize(
        "number, divisor, error", [(50, 0, ZeroDivisionError), (10, _square, TypeError), (str(5), 5, TypeError)],
    )
